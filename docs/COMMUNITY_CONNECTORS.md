...
|-------|-------------------|------------|------------------|
| **Music** | ListenBrainz (MusicBrainz OAuth) | `MUSICBRAINZ_CLIENT_ID`, `MUSICBRAINZ_CLIENT_SECRET` | `https://resolve-self.vercel.app/api/connectors/listenbrainz/callback` |
| **Music** | Navidrome (optional URL + user + pass on Profile) | — | No OAuth — credentials on Profile |
| **OSS** | GitHub (`Resolve connector` OAuth app) | `GITHUB_OAUTH_CLIENT_ID`, `GITHUB_OAUTH_CLIENT_SECRET` | `https://resolve-self.vercel.app/api/connectors/github/callback` |
...
RESOLVE_SYNC_URL=https://resolve-self.vercel.app/api/connectors/jellyfin/sync \
