  ...
  return (
    <div className={clsx(styles.workspaceFrame, "mt-12")}>
      <div className="relative flex min-h-12 items-center justify-between border-b border-white/[0.07] px-4 sm:px-5">
        <div className="flex items-center gap-3"><div className="flex gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-rose-400/60" /><span className="h-2.5 w-2.5 rounded-full bg-amber-300/60" /><span className="h-2.5 w-2.5 rounded-full bg-emerald-300/60" /></div><Image src={BRAND_LOGO_PATH} alt="" width={22} height={22} className="h-[22px] w-[22px] object-contain" /></div>
      ...
      <div className="relative grid lg:grid-cols-[180px_1fr]">
        <nav className="flex overflow-x-auto border-b border-white/[0.07] p-2 lg:flex-col lg:border-b-0 lg:border-r lg:p-3" aria-label="Product layer previews" role="tablist">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            return <button key={tab.id} id={`home-tab-${tab.id}`} aria-controls={`home-panel-${tab.id}`} type="button" role="tab" aria-selected={active === tab.id} onClick={() => setActive(tab.id)} className={clsx(styles.workspaceTab, "flex shrink-0 items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-xs transition", active === tab.id ? "bg-blue-400/[0.1] text-white ring-1 ring-blue-400/20" : "text-resolve-muted hover:bg-white/[0.04] hover:text-white")}><Icon className="h-3.5 w-3.5" />{tab.label}</button>;
          })}
        </nav>

        <div key={active} id={`home-panel-${active}`} aria-labelledby={`home-tab-${active}`} className={clsx(styles.tabPanel, "min-w-0 p-4 sm:p-6 lg:p-8")} role="tabpanel" aria-live="polite">
          <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
            <div className={clsx(styles.operationalCard, "rounded-2xl p-5 sm:p-6")}>
            ...
            <div className={clsx(styles.quietCard, "rounded-2xl p-5")}>
