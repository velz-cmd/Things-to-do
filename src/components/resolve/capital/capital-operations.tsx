...
export function CapitalOperations({ initialData = null }: { initialData?: CapitalBootstrap | null }) {
  ...
  const presentMoney = (value: string | null | undefined) => degraded ? "Unavailable" : money(value);
  const attention = [
    ...(degraded ? [{ id: "bootstrap", title: "Persisted Capital state is unavailable", detail: data.dataQuality?.message ?? "No financial values were inferred.", action: "Reload Capital", href: "/capital" }] : []),
    ...(!selectedWalletAddress ? [{ id: "wallet", title: "Funding wallet is not configured", detail: "Treasury actions require an eligible wallet in the canonical wallet registry.", action: "Review wallet setup", href: "/profile?view=wallets&returnTo=%2Fcapital" }] : []),
    ...(selectedWalletAddress && !selected && !degraded ? [{ id: "balance", title: "Selected wallet balance is unavailable", detail: data.dataQuality?.message ?? "The wallet selection is preserved, but no confirmed balance snapshot is available.", action: "Synchronize", href: null }] : []),
  ...
  const primary = degraded
    ? { label: "Retry Capital", action: () => void query.refetch() }
    ...
    <main className="min-h-screen bg-[#030711] text-[#f5f8fc] [background-image:linear-gradient(rgba(72,115,169,.04)_1px,transparent_1px),linear-gradient(90deg,rgba(72,115,169,.04)_1px,transparent_1px)] [background-size:40px_40px]">
      <div className="mx-auto w-full max-w-[1400px] space-y-4 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section aria-label="Treasury command band" className="grid overflow-hidden rounded-3xl border border-white/[0.09] bg-[#07111e]/95 shadow-[0_24px_80px_rgba(0,0,0,.22)] lg:grid-cols-[.9fr_1.1fr]">
          ...
          <dl className="grid grid-cols-2 gap-px bg-white/[0.06] sm:grid-cols-3"><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Available</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.availableMicroUsdc)}</dd></div><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Reserved</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.reservedMicroUsdc)}</dd></div><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Pending authorization</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.committedMicroUsdc)}</dd></div><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Pending settlement</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.pendingMicroUsdc)}</dd></div><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Claimable</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.claimableMicroUsdc)}</dd></div><div className="bg-[#081421] p-4"><dt className="text-[10px] uppercase tracking-wider text-[#71839b]">Settled · 30d</dt><dd className="mt-2 text-lg font-semibold tabular-nums text-white">{presentMoney(data.moneyState.settledThirtyDayMicroUsdc)}</dd></div></dl>
        </section>
        ...
        {view === "treasury" && <section className={`${panel} p-5`}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-cyan-300">Treasury</p><h2 className="mt-1 text-lg font-semibold text-white">Active wallet and movement controls</h2><p className="mt-1 text-xs text-[#8192aa]">Funding and transfer dialogs preserve the existing wallet handlers.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => openAddFunds()} disabled={degraded} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-500 px-4 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><CircleDollarSign className="h-4 w-4" />Add funds</button><button type="button" onClick={() => openSendFunds()} disabled={degraded} className={secondaryButton}><Send className="h-4 w-4" />Send</button><button type="button" onClick={() => void refresh()} disabled={busy} className={secondaryButton}><RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`} />Synchronize</button></div></div><div className="mt-5 grid gap-3 lg:grid-cols-[1.2fr_.8fr]"><div className="rounded-xl border border-white/[0.07] bg-[#081421] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium text-white">{data.wallets.selectedCapitalWallet === "connected" ? "Connected wallet" : "RESOLVE application wallet"}</p><p className="mt-1 break-all font-mono text-xs text-[#8192aa]">{selectedWalletAddress ?? "No persisted wallet"}</p></div><Status value={data.sync.balanceState} /></div><div className="mt-4 flex flex-wrap gap-2">{selectedWalletAddress && <button type="button" onClick={() => void navigator.clipboard.writeText(selectedWalletAddress)} className={secondaryButton}><Copy className="h-3.5 w-3.5" />Copy address</button>}{selectedWalletAddress && <a href={`https://testnet.arcscan.app/address/${selectedWalletAddress}`} target="_blank" rel="noreferrer" className={secondaryButton}>ArcScan<ExternalLink className="h-3.5 w-3.5" /></a>}</div></div><div className="rounded-xl border border-white/[0.07] bg-[#081421] p-4"><p className="text-[10px] uppercase tracking-wider text-[#71839b]">Portfolio provenance</p><p className="mt-2 text-2xl font-semibold tabular-nums text-white">{presentMoney(data.balances.portfolioTotalMicroUsdc)}</p><p className="mt-2 text-xs leading-5 text-[#8192aa]">Portfolio total is separate from selected-wallet spendable capital. State: {data.sync.balanceState}; network: {data.sync.networkHealth}.</p></div></div></section>}
        ...
        <details className="rounded-2xl border border-white/[0.07] bg-[#050c15] p-4 text-xs text-[#8192aa]"><summary className="cursor-pointer font-medium text-[#aab8c9]">Infrastructure diagnostics</summary><dl className="mt-4 grid gap-3 sm:grid-cols-5"><div><dt>Network</dt><dd className="mt-1 text-white">Arc Testnet · 5042002</dd></div><div><dt>Snapshot</dt><dd className="mt-1 text-white">{data.sync.balanceState}</dd></div><div><dt>Network health</dt><dd className="mt-1 text-white">{data.sync.networkHealth}</dd></div><div><dt>Data source</dt><dd className="mt-1 text-white">{data.dataQuality?.source === "server_fallback" ? "Auth-backed fallback" : "Persisted snapshot"}</dd></div><div><dt>Last refresh</dt><dd className="mt-1 text-white">{formatDate(data.sync.lastSuccessfulSyncAt)}</dd></div></dl><a href="https://testnet.arcscan.app" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1 text-blue-300">Open ArcScan<ExternalLink className="h-3.5 w-3.5" /></a></details>
      </div>
    </main>
  );
}
