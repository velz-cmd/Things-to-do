...
import { arcTestnet, RESOLVE_AGENT_ESCROW_ADDRESS } from "@/lib/arc/config";
...
export async function verifyAgentEscrowDeposit(params: {
  ...
  if (
    tx.to.toLowerCase() !== RESOLVE_AGENT_ESCROW_ADDRESS.toLowerCase()
  ) {
    return {
      ok: false as const,
      error: "Send USDC to the RESOLVE agent escrow address",
    };
  }
